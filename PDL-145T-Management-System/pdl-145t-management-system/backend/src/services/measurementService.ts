...
import { MeasurementCreateRequest, MeasurementUpdateRequest } from '../types/express.js';

const prisma = new PrismaClient();

export class MeasurementService {
  // Create a new measurement
  async createMeasurement(taskId: number, data: MeasurementCreateRequest) {
  ...
  async getMeasurementsByTask(taskId: number) {
  ...
  async updateMeasurement(id: number, data: MeasurementUpdateRequest) {
    const measurement = await prisma.measurement.findUnique({
      where: { MeasurementID: id },
    });
  ...
  async getMeasurementsBySite(siteId: string) {
    return await prisma.measurement.findMany({
      where: { SiteID: siteId },
      orderBy: { Date: 'desc' },
      include: {
  ...
  async getMeasurementsByType(measurementType: string) {
    return await prisma.measurement.findMany({
  ...
  async getMeasurementsPaginated(
    page: number = 1,
    limit: number = 10,
    sortBy: string = 'Date',
