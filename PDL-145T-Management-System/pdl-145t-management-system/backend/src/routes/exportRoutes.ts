 ...
 */
router.get('/report/:projectId/json', requireAuth, requireRole(['ADMIN', 'SUPERVISOR']), async (req: Request, res: Response) => {
  try {
 ...
 */
router.post('/report/:projectId/excel', requireAuth, requireRole(['ADMIN', 'SUPERVISOR']), async (req: Request, res: Response) => {
  try {
    const { projectId } = req.params;
 ...
 */
router.get('/report/:projectId/pdf', requireAuth, requireRole(['ADMIN', 'SUPERVISOR']), async (req: Request, res: Response) => {
  try {
    const { projectId } = req.params;
    const { includeAuditTrail, includeApprovals, includeReconciliation } = req.query;
 ...
 */
router.get('/audit-trail/:projectId', requireAuth, requireRole(['ADMIN']), async (req: Request, res: Response) => {
  try {
    const { projectId } = req.params;
 ...
 */
router.get('/reconciliation/:projectId', requireAuth, requireRole(['ADMIN', 'CQ']), async (req: Request, res: Response) => {
  try {
    const { projectId } = req.params;
