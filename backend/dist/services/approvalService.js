 ...
 */
export async function getApprovalQueue(userRole, userId) {
 ...
 */
export async function approveExpense(request) {
    ...
    try {
        ...
        const currentLevel = `LEVEL_${level}_${['RL', 'RC', 'CQ', 'CFEF'][level - 1]}`;
        ...
        const oldStatus = workflow[levelStatusKey];
        await logExpenseHistory(expenseId, `approval_level_${level}`, oldStatus, status, userId);
        return updated;
    }
    catch (error) {
 ...
 */
export async function isPaymentReady(expenseId) {
    try {
        const workflow = await prisma.approvalWorkflow.findUnique({
            where: { expenseId },
        });
        ...
        return (workflow.level1_status === 'Approved' &&
