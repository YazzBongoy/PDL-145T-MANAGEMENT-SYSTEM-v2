 ...
 */
export async function generateProjectReport(options) {
    try {
        ...
        if (options.includeApprovals) {
            ...
            reportData.approvalSummary = {
                ...
                approved: workflows.filter((w) => w.level1_status === 'Approved' &&
                    w.level2_status === 'Approved' &&
                    w.level3_status === 'Approved' &&
                    w.level4_status === 'Approved').length,
                pending: workflows.filter((w) => w.level1_status === 'Pending').length,
                rejected: workflows.filter((w) => w.level1_status === 'Rejected' ||
                    w.level2_status === 'Rejected' ||
                    w.level3_status === 'Rejected' ||
                    w.level4_status === 'Rejected').length,
            };
        }
 ...
 */
export async function generateExcelData(options) {
    ...
    if (reportData.budgetVariance) {
        sheets['Budget Variance'] = [
            ...
            ...reportData.budgetVariance.taskVariances.map((t) => [
                t.title,
                t.variance,
                t.variance < 0 ? 'Favorable' : 'Unfavorable',
            ]),
        ];
    }
    // Audit trail sheet
 ...
 */
export async function generatePdfData(options) {
    const reportData = await generateProjectReport(options);
    const lines = [];
    // Header
