...
export async function getProjectMetrics(projectId) {
    try {
        ...
        tasks.forEach((task) => {
            // Calculate spent
            const taskSpent = task.Expenses?.reduce((sum, exp) => sum + (exp.Cost || 0), 0) || 0;
        ...
        const budgetUtilization = Number(totalBudget) > 0 ? (Number(totalSpent) / Number(totalBudget)) * 100 : 0;
        ...
        return {
            ...
            remainingBudget: Math.max(0, Number(totalBudget) - Number(totalSpent)),
...
export async function getTaskMetrics(projectId) {
    try {
        ...
        return tasks.map((task) => {
            const actualCost = task.Expenses?.reduce((sum, exp) => sum + (exp.Cost || 0), 0) || 0;
 ...
 */
export async function getBudgetVariance(projectId) {
    try {
 ...
 */
export async function generateBurndownChart(projectId) {
    try {
 ...
 */
export async function getApprovalQueueMetrics(projectId) {
    try {
        ...
        const metrics = {
            totalPending: workflows.filter((w) => w.level1_status === 'Pending' || w.level2_status === 'Pending' || w.level3_status === 'Pending' || w.level4_status === 'Pending').length,
            ...
            approved: workflows.filter((w) => w.level1_status === 'Approved' && w.level2_status === 'Approved' && w.level3_status === 'Approved' && w.level4_status === 'Approved').length,
            rejected: workflows.filter((w) => w.level1_status === 'Rejected' || w.level2_status === 'Rejected' || w.level3_status === 'Rejected' || w.level4_status === 'Rejected').length,
 ...
 */
export async function getReconciliationMetrics(projectId) {
    try {
