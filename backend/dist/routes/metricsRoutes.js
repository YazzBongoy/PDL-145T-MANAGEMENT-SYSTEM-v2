...
import { getProjectMetrics, getTaskMetrics, getBudgetVariance, generateBurndownChart, getApprovalQueueMetrics, getReconciliationMetrics, } from '../services/metricsService.js';
 ...
 */
router.get('/project/:projectId/tasks', requireAuth, async (req, res) => {
    try {
        const { projectId } = req.params;
 ...
 */
router.get('/project/:projectId/budget-variance', requireAuth, async (req, res) => {
    try {
        const { projectId } = req.params;
 ...
 */
router.get('/project/:projectId/burndown', requireAuth, async (req, res) => {
    try {
        const { projectId } = req.params;
 ...
 */
router.get('/approvals', requireAuth, requireRole(['ADMIN', 'SUPERVISOR']), async (req, res) => {
    try {
        const { projectId } = req.query;
        const metrics = await getApprovalQueueMetrics(projectId ? parseInt(projectId, 10) : undefined);
        res.json({
            success: true,
            data: metrics,
        });
 ...
 */
router.get('/reconciliation', requireAuth, requireRole(['ADMIN', 'SUPERVISOR']), async (req, res) => {
    try {
        const { projectId } = req.query;
        const metrics = await getReconciliationMetrics(projectId ? parseInt(projectId, 10) : undefined);
        res.json({
            success: true,
            data: metrics,
        });
