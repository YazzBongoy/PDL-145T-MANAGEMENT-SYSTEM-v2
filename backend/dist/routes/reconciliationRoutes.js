...
import { createReconciliationAudit, matchInvoiceToPO, reportDiscrepancy, resolveDiscrepancy, getPendingReconciliations, getReconciliationHistory, generateReconciliationReport, getActiveDiscrepancies, } from '../services/reconciliationService.js';
 ...
 */
router.post('/audit', requireAuth, requireRole(['ADMIN', 'CQ']), async (req, res) => {
    try {
 ...
 */
router.get('/pending', requireAuth, requireRole(['ADMIN', 'CQ']), async (req, res) => {
    try {
 ...
 */
router.get('/discrepancies', requireAuth, requireRole(['ADMIN', 'CQ']), async (req, res) => {
    try {
 ...
 */
router.post('/:auditId/match', requireAuth, requireRole(['ADMIN', 'CQ']), async (req, res) => {
    try {
        ...
        const result = await matchInvoiceToPO(parseInt(auditId, 10), invoiceId, purchaseOrderId, userId);
        if (result.matched) {
            res.json({
                success: true,
 ...
 */
router.post('/:auditId/discrepancy', requireAuth, requireRole(['ADMIN', 'CQ']), async (req, res) => {
    try {
        const { auditId } = req.params;
        ...
        const audit = await reportDiscrepancy(parseInt(auditId, 10), discrepancyReason, userId);
        res.json({
            success: true,
 ...
 */
router.patch('/:auditId/resolve', requireAuth, requireRole(['ADMIN', 'CQ']), async (req, res) => {
    try {
        const { auditId } = req.params;
        ...
        const audit = await resolveDiscrepancy(parseInt(auditId, 10), resolutionNotes, userId);
        res.json({
            success: true,
 ...
 */
router.get('/report', requireAuth, requireRole(['ADMIN', 'SUPERVISOR']), async (req, res) => {
    try {
        ...
        const report = await generateReconciliationReport(new Date(startDate), new Date(endDate));
        res.json({
            success: true,
            data: report,
        });
