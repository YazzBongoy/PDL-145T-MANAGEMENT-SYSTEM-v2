...
async function recalcSiteTEP(siteId: string): Promise<number> {
  ...
  for (const l1 of l1tasks) {
    ...
    for (const l2 of l2tasks) {
      ...
      let tepL2 = l3tasks.length > 0
        ? l3tasks.reduce((s, t) => s + t.progressPercentage, 0) / l3tasks.length
        : l2.progressPercentage;
  ...
  await prisma.site.update({ where: { SiteID: siteId }, data: { tepGlobal: tep } });
  return tep;
}
