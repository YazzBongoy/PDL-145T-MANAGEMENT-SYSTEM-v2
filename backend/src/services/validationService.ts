...
import { ValidationCreateRequest, ValidationUpdateRequest } from '../types/express.js';

const prisma = new PrismaClient();

export class ValidationService {
  // Create a new validation
  async createValidation(taskId: number, data: ValidationCreateRequest) {
  ...
  async getValidationsByTask(taskId: number) {
  ...
  // Update a validation
  async updateValidation(id: number, data: ValidationUpdateRequest) {
    const validation = await prisma.validation.findUnique({
      where: { ValidationID: id },
    });
  ...
  async getValidationsBySite(siteId: string) {
    return await prisma.validation.findMany({
      where: { SiteID: siteId },
      orderBy: { Timestamp: 'desc' },
      include: {
  ...
  async getValidationsByStatus(status: 'Pending' | 'Approved' | 'Rejected') {
    return await prisma.validation.findMany({
      where: { Status: status },
      orderBy: { Timestamp: 'desc' },
      include: {
  ...
  async approveValidation(id: number, approvedBy: string) {
    const validation = await prisma.validation.findUnique({
      where: { ValidationID: id },
    });
  ...
  // Reject a validation
  async rejectValidation(id: number, rejectedBy: string, reason?: string) {
    const validation = await prisma.validation.findUnique({
      where: { ValidationID: id },
    });
